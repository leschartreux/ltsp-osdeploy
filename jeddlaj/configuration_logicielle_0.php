...
entete("Gérard Milhaud & Frédéric Bloise : La.Firme@esil.univ-mrs.fr", "CSS/g.css", "JeDDLaJ : Configuration Logicielle - Etape 0");
include ("DBParDefaut.php");
ConnectMySQL($GLOBALS['host'], $GLOBALS['user'], $GLOBALS['pwd']);
SelectDb($GLOBALS['db']);

print("<CENTER><H1>Configuration Logicielle - Etape 0</H1></CEnTER>\n");

print("<CENTER>\n");
print("<TABLE>\n");
print("<FORM METHOD=GET NAME=\"form\" ACTION=\"configuration_logicielle_1.php\">\n");
...
print("</TR>\n</TABLE>\n");
mysql_free_result($result);
print("<BR>Distribution : <SELECT name=\"id_os\">\n");
# 21-06-2006 : On ne montre maintenant que les distributions visibles 
$request = "SELECT DISTINCT id_logiciel AS id_os,nom_os,nom_logiciel,version FROM logiciels, images_de_base WHERE id_logiciel=id_os AND visible=\"oui\" ORDER BY nom_os,nom_logiciel,version";
$result = mysql_query($request);
$nom_os="";
for ($i=0;$i<mysql_num_rows($result);$i++) {
  $line = mysql_fetch_array($result);
  ...
  print("<OPTION value=\"".$line["id_os"]."\">".$line["nom_logiciel"]." ".$line["version"]."</OPTION>\n");
}
print("</SELECT>");
