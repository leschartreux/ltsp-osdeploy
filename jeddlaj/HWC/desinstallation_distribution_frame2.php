...
if (isset($_POST["desinstall"])) {
	if (isset($_POST["nom_dns_checked"])) {
		...
		if ($expired) {
  		print("La sélection a expiré.<BR>\n");
			...
			mysql_query($request);
			print ("<P><CENTER><I>Mofications insérées dans la base. <FONT COLOR=RED>ATTENTION :</FONT> La distribution a été supprimée sur les ordinateurs concernés au niveau de la base. Elle ne sera réellement supprimée sur les ordinateurs concernés qu'après installation d'une nouvelle distribution...</I></CENTER></P>\n");
		}
	...
	} else print("Cette distribution n'est installée sur aucun ordinateur du groupe sélectionné.<BR>\n");
}
print("</CENTER>\n");
