...
$mode == "edition" ? entete("Gérard Milhaud & Frédéric Bloise : La.Firme@esil.univ-mrs.fr", "CSS/g.css", "JeDDLaJ : Édition postinstall script ($action)") : entete("Gérard Milhaud & Frédéric Bloise : La.Firme@esil.univ-mrs.fr", "CSS/g.css", "JeDDLaJ : Suppression postinstall script ($action)");
include ("DBParDefaut.php");
ConnectMySQL($GLOBALS['host'], $GLOBALS['user'], $GLOBALS['pwd']);
SelectDb($GLOBALS['db']);
$mode == "edition" ? print("<CENTER><H1>Édition de postinstall scripts</H1></CENTER>\n") : print("<CENTER><H1>Suppression de postinstall scripts</H1></CENTER>\n");

switch ($action)
{
	case "ChoixPostInstallScript":
	        $request="SELECT * FROM postinstall_scripts";
		$result=mysql_query($request);
		$nb_scripts = mysql_num_rows($result);
		print("<H2>Choix postinstall script</H2>");
		$mode == "edition" ? EnteteFormulaire("POST","editer_ou_supprimer_postinstall_script.php?action=EditScript&mode=$mode") : EnteteFormulaire("POST","editer_ou_supprimer_postinstall_script.php?action=Validation&mode=$mode");
		...
		while ($ligne = mysql_fetch_array($result))
		{
			...
			if ($mode == "edition")
			{
				# Edition : un seul postinstall script à la fois donc RADIO
				print("\n</TD>\n<TD>\n <INPUT TYPE=RADIO NAME=\"id_script\" VALUE=\"$ligne[id_script]\">\n </TD>\n");
			}
			else
			{
				# Suppression : eventuellement plusieurs postinstall script à la fois donc CHECKBOX
				print("\n</TD>\n<TD>\n <INPUT TYPE=CHECKBOX NAME=\"checked_pis[".$i++."]\" VALUE=\"$ligne[id_script]\">\n </TD>\n");
			}
			print("</TR>\n");
		}
	...
	case "EditScript":
		...
		if (!isset($id_script))
		{
			print ("<P><I><FONT COLOR=RED>ATTENTION : Vous n'avez choisi aucun postinstall_script</FONT>. Utilisez le bouton <TT>BACK</TT> de votre navigateur pour faire une sélection valide.</I></P>");
		}
		else
		{
	        	$request="SELECT * FROM postinstall_scripts WHERE id_script=\"$id_script\"";
			$result=mysql_query($request);
			...
			print("<H2>Postinstall script à éditer</H2>");
			print("<FORM NAME=\"edit_postinst\" METHOD=\"POST\" ACTION=\"editer_ou_supprimer_postinstall_script.php?action=Validation&mode=$mode\">");
			EnteteTable("BORDER=0 CELLPADDING=2 CELLSPACING=5");
			print("<TR><TD><I>Nom postinstall script</I> </TD><TD>: <INPUT TYPE=TEXT NAME=nom_script SIZE=50 VALUE=\"$nom_script\"></TD></TR>\n");
			print("<TR><TD><I>Répertoire</I> </TD><TD>: <FONT SIZE=-1 COLOR=GREEN><I>$RemboPostInstScriptsDir</I></FONT><INPUT TYPE=TEXT NAME=repertoire SIZE=50 VALUE=\"$repertoire\"></TD></TR>\n");
			print("<TR>\n<TD><I>Applicable à : </I></TD></TR>\n");
			print("<TR><TD><I>&nbsp;&nbsp;&nbsp; <IMG ALIGN=CENTER SRC=ICONES/purpleball.png> un groupe  </I></TD>\n");
			print("<TD>: <SELECT name=\"nom_groupe\" onChange=\"javascript:document.edit_postinst.nom_dns.selectedIndex=0\">\n");
			if ($applicable_a == "nom_dns")
			{
				...
				print("<TD>: <SELECT name=\"nom_dns\" onChange=\"javascript:document.edit_postinst.nom_groupe.selectedIndex=0\">\n");
				print("<OPTION value=\"\"></OPTION>\n");
			...
			FinTable();
			print("<H2>Logiciels associés à ce postinstall script</H2>");
			# On recupère les logiciels associés à ce script
			$request="SELECT id_logiciel from pis_est_associe_a where id_script=\"$id_script\"";
			$result=mysql_query($request);
			$i = 1;
			while ($ligne = mysql_fetch_array($result))
			{
	...
	case "Validation":
		...
		if ($mode == "edition")
		{
			...
			# On ajoute un slash final au chemin s'il n'y est pas déjà, sauf si répertoire est vide sinon on aurait $RemboPostInstScriptsDir/ (donc deux slashes finaux...)
		...
		switch ($mode)
		{
			case "edition" :
				...
				# Le nom du postinstall script n'est pas précisé
				if ($nom_script == "")
				{
					print("<P><I><FONT COLOR = RED>ATTENTION DANGER : </FONT>Vous n'avez pas donné de nom au postinstall script que vous souhaitez ajouter... Veuillez utiliser le bouton Back/Précédent de votre navigateur pour modifier votre entrée.</I></P>\n");
					break;
				}
				# Pas de valeur d'application... : on interdit l'ajout
				if ($nom_groupe == "" and $nom_dns == "")
				{
					print("<P><I><FONT COLOR = RED>ATTENTION DANGER : </FONT>Vous n'avez pas spécifié à quoi s'applique ce postinstall script... Veuillez utiliser le bouton Back/Précédent de votre navigateur pour modifier votre entrée.</I></P>\n");
					break;
				}
		
				# Reste des cas nécessitant des requêtes
				# On vérifie qu'on n'insère pas un postinstall script déjà existant i.e. de même nom dans le même répertoire mais avec un id_script
				# qui n'est pas celui du script courant...
				$request = "SELECT COUNT(*) AS total FROM postinstall_scripts WHERE nom_script=\"$nom_script\" AND repertoire=\"$repertoire\" AND id_script<>\"$id_script\"";
				$result = mysql_query($request);
				$line = mysql_fetch_array($result);
				$postinstscript_meme_rep_deja_existant = ($line["total"] != 0);
				mysql_free_result($result);
				if ($postinstscript_meme_rep_deja_existant)
				{
					print("<P><I><FONT COLOR = RED>ATTENTION DANGER : </FONT>Un postinstall script, qui n'est pas celui que vous éditez, de nom <FONT COLOR=RED>$nom_script</FONT> existe déjà dans le répertoire $RemboPostInstScriptsDir<FONT COLOR=RED>$repertoire</FONT> ... Veuillez utiliser le bouton Back/Précédent de votre navigateur pour modifier votre entrée.</I></P>\n");
					break;
				}

		 		# OK, on peut attaquer la base
				# 1. On corrige dans la table postinstall_scripts
				...
				mysql_query("UPDATE postinstall_scripts SET id_script=\"$id_script\", nom_script=\"$nom_script\", repertoire=\"$repertoire\", applicable_a=\"$applicable_a\", valeur_application=\"$valeur_application\" WHERE id_script=\"$id_script\"");
				print("<P><I>Le postinstall script <FONT COLOR=RED>$nom_script</FONT> (id_script=$id_script) a été corrigé sans stress dans la table des postinstall scripts.</I></P>\n");
				# 2. On va réinsérer dans la table pis_est_associe_a
				# 2.1 On détruit les anciennes associations de logiciels
				mysql_query("DELETE FROM pis_est_associe_a where id_script=\"$id_script\"");
				...
				$logiciels_associes .= "</UL>\n";
				mysql_free_result($result);
				print("<I><P>Le script de postinstall $nom_script a été associé dans la base aux logiciels :</P>".$logiciels_associes."</I>");
				break;
				################################## LA SUITE EST A FAIRE !!!!!!!
			case "suppression" :
				# Le nombre de postinstall scripts choisis est non nul
				if (count($checked_pis) > 0)
				{
					$nb_scripts_concernes=0;
					$liste_id_scripts_concernes="";
					for($i=1;$i<=$nb_scripts;$i++)
					{
						if (isset($checked_pis[$i]))
						{
							$nb_scripts_concernes++;
							$id_scripts_concernes[$nb_scripts_concernes] = $checked_pis[$i];
							...
							$tables_concernees = array("postinstall_scripts", "pis_est_associe_a");
							...
							$request = "SELECT id_script, nom_script, repertoire FROM postinstall_scripts".$clause_where;
							$result = mysql_query($request);
							$scripts_detruits = "<UL>\n";
							while ($ligne = mysql_fetch_array($result))
							{
				...
				}# end if(count($checked_pis) > 0)
				else
				{
					print ("<P><I><FONT COLOR=RED>ATTENTION : Vous n'avez choisi aucun postinstall script</FONT>. Utilisez le bouton <TT>BACK</TT> de votre navigateur pour faire une sélection valide.</I></P>");
				}
		}
		break;
}
