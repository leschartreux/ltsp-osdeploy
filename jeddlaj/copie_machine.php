...
print("<TITLE>JeDDLaJ : Copie Machine</TITLE>\n");
...
if (isset($_GET["nom_dns"])) {
	...
	insereLigne(2,"<i>Adrese IP : ".$line["adresse_ip"]."</i>","true","false");
	insereLigne(2,"<i>Masque de sous-réseau : ".$line["netmask"]."</i>","true","false");
	insereLigne(2,"<i>Passerelle : ".$line["gateway"]."</i>","true","false");
	insereLigne(2,"<i>Adresse MAC : ".$line["adresse_mac"]."</i>","true","false");
	insereLigne(2,"<i>Nom NetBios : ".$line["nom_netbios"]."</i>","true","false");
	...
	insereLigne(2,"<i>Numéro de Série : ".$line["numero_serie"]."</i>","true","false");
	...
	insereLigne(1,"Composants","false","false");
	insereLigne(2,"Signature : ".$line["signature"],"true","false");
	$request1 = "SELECT type,nom FROM composant_est_installe_sur AS a, composants AS b WHERE a.id_composant=b.id_composant AND nom_dns=\"$nom_dns\" ORDER BY type";
	$result1 = mysql_query($request1);
	for ($i1=0;$i1<mysql_num_rows($result1);$i1++) {
	  $line1 = mysql_fetch_array($result1);
		insereLigne(2,$line1["type"]." : ".$line1["nom"],"true","false");
	}
	...
	mysql_free_result($result1);
	insereLigne(1,"Groupes","false","false");
	...
	$request1 = "SELECT id_logiciel,num_disque,num_partition,nom_os,nom_logiciel,version,etat_idb,cache,boot_options FROM idb_est_installe_sur AS a, images_de_base AS b, logiciels  WHERE nom_dns=\"$nom_dns\" AND a.id_idb=b.id_idb AND id_os=id_logiciel ORDER BY num_disque,num_partition";
	$result1 = mysql_query($request1);
	for ($i1=0;$i1<mysql_num_rows($result1);$i1++) {
	  $line1 = mysql_fetch_array($result1);
		insereLigne(2,"<A HREF='javascript:parent.location.href=\\\"examine_distribution.php?id_logiciel=".$line1["id_logiciel"]."\\\"'>".$line1["nom_logiciel"]." ".$line1["version"]."</A> <I>( $line1[etat_idb] )</I>","false","false");
		...
		insereLigne(3,"Logiciels","false","false");
		...
		for ($i2=0;$i2<mysql_num_rows($result2);$i2++) {
		  $line2 = mysql_fetch_array($result2);
			insereLigne(4,"<A HREF='javascript:parent.location.href=\\\"examine_logiciel.php?id_logiciel=".$line2["id_logiciel"]."\\\"'>".$line2["nom_logiciel"]." ".$line2["version"]."</A> <I>( $line2[etat_package] )</I>","true","false");
		}
		mysql_free_result($result2);
	}
	mysql_free_result($result1);
...
print("<HTML>\n");
print("<FRAMESET frameborder=0 border=0 framespacing=0 rows=\"200,*\">\n");
print("  <FRAME SRC=\"copie_machine_frame1.php?nom_dns=$nom_dns\" name=\"examine_machine\" SCROLLING=\"NO\" marginwidth=0 marginheight=0  noresize>\n");
