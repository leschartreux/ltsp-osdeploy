...
/******************************************************
 *                                                    *
 *      traitement du formulaire de l'étape 6         *
 *                                                    *
 ******************************************************/
$packageDiff = false;
    ...
    while ($row = current($_POST)) {
        ...
        if ($row === 'oui') {
            ...
            $sql = 'UPDATE `'.$prefix.'_packages` '.
                   'SET id_package = '.$idPackage.' '.
                   'WHERE id_package = '.$idPackageDist;
...
if ($packageDiff) {
    ...
    $content .= 'Pour passer à l\'étape suivante vous devez effectuer une des actions suivantes pour chaque package concerné : <br \>';

    $content .= '<ul>';

    $content .= '<li>Changer la spécificité ou la valeur de la spécificité du package local ou distant. (Les changements sur la base distante nécéssitent de réimporter la base)</li>';

    $content .= '<li>Supprimer le package local</li>';
    ...
    /*********************************************
     *                                           *
     *      Formulaire de l'étape 7              *
     *                                           *
     *********************************************/
    $content .= '<h2>Recherche des packages identiques</h2>';



    $sql = 'INSERT INTO `fusion` (prefixe, etape) VALUES ("'.$prefix.'", 7)';
    ...
    if ($nbResult > 0) {
        ...
        $content .= '<p>Dans cette étape, on recherche les packages ayant la même spécificité mais une valeur de spécificité différente.<br />Dans ce formulaire : cochez oui pour remplacer les packages distants par les packages locals.</p>';

        $content .= '<form action="step8.php" method="post">';
        ...
        $content .= '<p><a href="step8.php">Suivant</a></p>';
    }
 }
